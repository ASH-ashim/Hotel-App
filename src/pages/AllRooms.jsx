...
return(
    <label className="flex gap-3 items-center cursor-pointer mt-2 txt-sm">
        <input type="checkbox" cheecked={selected}
...
const RadioButton = ({label, selected = false, onChange =() => { } }) => {
    return(
        <label className="flex gap-3 items-center cursor-pointer mt-2 txt-sm">
            <input type="radio" name="sort option" cheecked={selected}
...
const AllRooms = () => {
    ...
    return (
    <div className="flex flex-col-reverse mt-25 lg:flex-row bg-gray-600 items-start justify-between
    pt-28 md:pt-25 px-4 md:px-16 lg:px-24">
    <div>
    <div className="flex flex-col items-start text-left">

        <h1 className="font-playfair text-[#ffffc7] text-4xl md:text-[40px]">Hotel Rooms</h1>
        <p className="text-sm md:text-base text-gray-200/90 mt-2
        max-w-174">Explore our curated selection of rooms designed for comfort, luxury, and unforgettable experiences — 
    ...
    {roomsDummyData.map((room) =>(
        ...
        src={room.images[0]} alt="Hotel" title="View Room Details"
        className="max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover
        cursor-pointer" />

        <div className="md:w-1/2 flex flex-col gap-2">

            <p className="text-gray-300">{room.hotel.city}</p>
     ...
     {/* Room Ammenities */}
        ...
        {room.amenities.map((item, index)=>(
            ...
            <p className="text-xl font-medium text-gray-300">NPR {room.pricePerNight} /Night</p>
        </div>
        </div>
    ))}
    </div>
...
<div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-500`}>
        <div className="px-5 pt-5">
            ...
            {PriceRange.map((range, index)=> (
                <CheckBox  key={index} label={`NPR ${range}`} />
            ))}
        </div>
